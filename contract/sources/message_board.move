module medical_records_addr::medical_records {
    ...
    struct MedicalReport has store {
        report_hash: string::String, // IPFS or encrypted hash
        timestamp: u64,
        emergency_flag: bool,
    }
    ...
    /// Register patient
    public entry fun register_patient(user: &signer) {
    ...
    public entry fun register_doctor(admin: &signer, doctor: address) {
        let registry = borrow_global_mut<Registry>(signer::address_of(admin));
        table::add(&mut registry.verified_doctors, doctor, true);
    }

    /// Upload medical report
    public entry fun upload_report(user: &signer, report_hash: string::String, emergency_flag: bool) {
        let addr = signer::address_of(user);
        let registry = borrow_global_mut<Registry>(addr);
        let profile = table::borrow_mut(&mut registry.users, addr);
        let report = MedicalReport {
            report_hash,
            timestamp: 0, // Replace with blockchain timestamp if needed
            emergency_flag,
        };
    ...
    /// Grant doctor access
    public entry fun authorize_doctor(user: &signer, doctor: address) {
        let addr = signer::address_of(user);
        let registry = borrow_global_mut<Registry>(addr);
        let profile = table::borrow_mut(&mut registry.users, addr);
    ...
    public fun view_reports(doctor: &signer, patient: address): vector<MedicalReport> acquires Registry {
        ...
        let authorized = vector::contains(&profile.authorized_doctors, signer::address_of(doctor));
    ...
    public fun emergency_access(doctor: &signer, patient: address): vector<MedicalReport> acquires Registry {
        ...
        let profile = table::borrow(&registry.users, patient);
        let mut emergency_reports = vector::empty<MedicalReport>();
